...
exports.commitIdOf = commitIdOf;
exports.changeIdOf = changeIdOf;
const exec_js_1 = require("../exec.js");
function jjIdArgv(cwd, ...subcommand) {
    return [
        '--repository',
        cwd,
        '--no-pager',
...
function commitIdOf(cwd, changeId) {
    const args = jjIdArgv(cwd, 'log', '-r', changeId, '-T', 'commit_id', '--no-graph', '-n', '1');
    const r = (0, exec_js_1.vcsExec)(cwd, 'jj', args);
    if (r.exitCode !== 0) {
        throw new exec_js_1.VcsExecError(`jj-id.commitIdOf failed for change ${changeId}: ${r.stderr || r.stdout}`, {
            exitCode: r.exitCode,
            stdout: r.stdout,
            stderr: r.stderr,
            timedOut: r.timedOut,
...
function changeIdOf(cwd, commitId) {
    const args = jjIdArgv(cwd, 'log', '-r', commitId, '-T', 'change_id', '--no-graph', '-n', '1');
    const r = (0, exec_js_1.vcsExec)(cwd, 'jj', args);
    if (r.exitCode !== 0) {
        throw new exec_js_1.VcsExecError(`jj-id.changeIdOf failed for commit ${commitId}: ${r.stderr || r.stdout}`, {
            exitCode: r.exitCode,
            stdout: r.stdout,
            stderr: r.stderr,
            timedOut: r.timedOut,

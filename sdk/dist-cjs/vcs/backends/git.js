...
const git_rev_js_1 = require("../parse/git-rev.js");
...
function createGitAdapter(cwd) {
    ...
    const commit = (input) => {
        ...
        if (input.files && input.files.length > 0) {
            ...
            const headExists = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--verify', '--quiet', 'HEAD']).exitCode === 0;
            const resetRes = (0, exec_js_1.execGit)(cwd, ['read-tree', headExists ? 'HEAD' : '--empty']);
            ...
            // guard.
            const addRes = (0, exec_js_1.execGit)(cwd, ['add', '-A', '--', ...input.files]);
        ...
        // more.
        const commitRes = (0, exec_js_1.execGit)(cwd, args);
    ...
    const bookmarks = Object.freeze({
        list: () => {
            const r = (0, exec_js_1.execGit)(cwd, ['branch', '--format=%(refname:short)']);
            if (r.exitCode !== 0)
                return [];
        ...
        create: (name, rev, _opts) => {
            ...
            const r = (0, exec_js_1.execGit)(cwd, ['branch', '--', actualName, (0, git_rev_js_1.toGitRev)(rev)]);
            if (r.exitCode !== 0) {
        ...
        move: (name, rev, _opts) => {
            ...
            const actualName = name;
            (0, refs_validator_js_1.validateRefname)(actualName);
            const r = (0, exec_js_1.execGit)(cwd, ['branch', '-f', '--', actualName, (0, git_rev_js_1.toGitRev)(rev)]);
            if (r.exitCode !== 0) {
        ...
        exists: (name, _opts) => {
            ...
            const actualName = name;
            (0, refs_validator_js_1.validateRefname)(actualName);
            const r = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--verify', '--quiet', actualName]);
            return r.exitCode === 0;
        },
    ...
    const currentBookmarks = () => {
        const r = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--abbrev-ref', 'HEAD']);
        if (r.exitCode !== 0)
            return [];
    ...
    const countCommits = (opts) => {
        ...
        const r = (0, exec_js_1.execGit)(cwd, ['rev-list', '--count', target]);
        if (r.exitCode !== 0)
            return 0;
    ...
    const refExists = (rev) => {
        ...
        const r = (0, exec_js_1.execGit)(cwd, ['cat-file', '-t', (0, git_rev_js_1.toGitRev)(rev)]);
        return r.exitCode === 0;
    };
    const isIgnored = (p) => {
        ...
        const r = (0, exec_js_1.execGit)(cwd, ['check-ignore', '-q', '--no-index', '--', p]);
        return r.exitCode === 0;
    };
    const remotes = () => {
        const r = (0, exec_js_1.execGit)(cwd, ['remote']);
        if (r.exitCode !== 0)
            return [];
        return r.stdout.split('\n').filter(Boolean).map((s) => s.trim());
    };
    ...
    const workspace = Object.freeze({
        add: (input) => {
            ...
            const head = (0, exec_js_1.execGit)(input.path, ['rev-parse', 'HEAD']);
            return {
                path: input.path,
        ...
        forget: (path) => {
            const r = (0, exec_js_1.execGit)(cwd, ['worktree', 'remove', path]);
            if (r.exitCode !== 0) {
        ...
        list: () => {
            ...
            const result = (0, worktree_list_js_1.readWorktreeList)(cwd);
            if (!result.ok)
                return [];
        ...
        context: () => {
            const top = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--show-toplevel']);
            const gd = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--git-dir']);
            const cd = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--git-common-dir']);
        ...
        prune: () => (0, exec_js_1.execGit)(cwd, ['worktree', 'prune']),
        reap: (opts) => {
            ...
            for (const entry of entries) {
                ...
                if (!name.startsWith(opts.phaseNamePrefix))
                    continue;
                const removeRes = (0, exec_js_1.execGit)(cwd, ['worktree', 'remove', entry.path]);
    ...
    const findConflicts = (opts) => {
        if (opts.scope === 'all') {
            ...
            const r = (0, exec_js_1.execGit)(cwd, ['ls-files', '--unmerged']);
        ...
        const r = (0, exec_js_1.execGit)(cwd, ['diff', '--check']);
        if (r.exitCode === 0)
            return [];
        const paths = new Set();
        for (const line of r.stdout.split('\n')) {
    ...
    const push = (opts = {}) => {
        ...
        if (opts.ref)
            args.push((0, git_rev_js_1.toGitRev)(opts.ref));
        return (0, exec_js_1.execGit)(cwd, args);
    };
    const fetch = (opts = {}) => {
        const args = ['fetch'];
        if (opts.remote)
    ...
    const gitOnly = Object.freeze({
        createAnnotatedTag: (name, message, rev) => {
            const r = (0, exec_js_1.execGit)(cwd, ['tag', '-a', name, '-m', message, (0, git_rev_js_1.toGitRev)(rev)]);
            if (r.exitCode !== 0) {
        ...
        revertAbort: () => {
            return (0, exec_js_1.execGit)(cwd, ['revert', '--abort']);
        },
        ...
        version: () => {
            ...
            const r = (0, exec_js_1.execGit)(cwd, ['--version']);
            if (r.exitCode !== 0) {
        ...
        // shapes when needed.
        init: () => {
            const r = (0, exec_js_1.execGit)(cwd, ['init']);
            if (r.exitCode !== 0) {
        ...
        configGet: (key) => {
            ...
            const r = (0, exec_js_1.execGit)(cwd, ['config', '--get', key]);
            if (r.exitCode === 0)
                return r.stdout.trim();
            if (r.exitCode === 1)
                return null;
        ...
        configSet: (key, value) => {
            const r = (0, exec_js_1.execGit)(cwd, ['config', key, value]);
            if (r.exitCode !== 0) {
        ...
        gitDir: () => {
            const r = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--git-dir']);
            if (r.exitCode !== 0) {
        ...
        gitCommonDir: () => {
            const r = (0, exec_js_1.execGit)(cwd, ['rev-parse', '--git-common-dir']);
            if (r.exitCode !== 0) {
    ...
    const testOnly = Object.freeze({
        snapshot: () => {
            const head = (0, exec_js_1.execGit)(cwd, ['rev-parse', 'HEAD']);
            if (head.exitCode !== 0) {
                throw new Error(`__vcsTestOnly.snapshot: rev-parse HEAD failed: ${head.stderr}`);
            }
            (0, exec_js_1.execGit)(cwd, ['update-ref', 'refs/gsd/test-snapshot', head.stdout]);
            return { id: head.stdout, kind: 'git' };
        },
        restore: (handle) => {
            const reset = (0, exec_js_1.execGit)(cwd, ['reset', '--hard', handle.id]);
            if (reset.exitCode !== 0) {
                throw new Error(`__vcsTestOnly.restore: reset failed: ${reset.stderr}`);
            }
            (0, exec_js_1.execGit)(cwd, ['clean', '-fdx']);
        },
    });

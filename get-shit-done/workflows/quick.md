...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   # --amend, git log --follow) is the upstream shell fallback. On jj-port it
   # is unreachable — gsd-sdk is always installed, so the `if command -v
   # gsd-sdk` arm above always runs — but it lives in source until the
   # wave-cleanup adapter verbs land and the entire fallback can be dropped.

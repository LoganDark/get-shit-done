...
function computeUiSafetyGate(projectDir: string, phase: string): {
  ...
  // (c) Check whether any UI files were changed in the last landed commit.
  // 19-12 next-merge port (AUDIT-01): adapter-routed — the diff of the head
  // revision against its parent (git: `git diff HEAD~1..HEAD --name-only`;
  // jj: `jj diff -r @-..@ --name-only`, a singleton revset = the head change).
  // Known limitation: multi-plan waves may need the wave-start commit for full coverage.
  let hasUiFiles = false;
  try {
    const vcs = createVcsAdapter(projectDir);
    const changed = vcs.diff({
      nameOnly: true,
      rev: expr.range(expr.ancestor(1), expr.head()),
    }).nameOnly;                                                              // (was: git diff --name-only HEAD~1 HEAD)
    hasUiFiles = changed.some((f) =>
      f.trim() && (UI_FILE_EXTENSIONS_RE.test(f) || UI_PATH_PATTERNS_RE.test(f)),
    );
  } catch { /* VCS unavailable or no prior commit — treat as no UI files changed */ }
...
function cmdTddReviewCheckpoint(projectDir: string, args: string[], raw: boolean): void {
  ...
  // For each TDD plan, extract the plan ID (padded plan number) and check the
  // commit history for the RED/GREEN/REFACTOR gate commits.
  // 19-12 next-merge port (AUDIT-01): adapter-routed — one vcs.log() sweep
  // replaces the per-plan `git log --grep` trio (was: git log --oneline
  // --grep='^<type>(planId):'); the gate probe matches subject+body line
  // starts in JS, mirroring the recentCommitMessages() pattern above. The
  // 500-entry bound is generous for a single phase's TDD gate commits.
  let commitMessageLines: string[] = [];
  try {
    const vcs = createVcsAdapter(projectDir);
    commitMessageLines = vcs.log({ maxCount: 500 })
      .flatMap((e) => `${e.subject || ''}\n${e.body || ''}`.split('\n'));
  } catch { /* VCS unavailable — every gate probe below reports missing */ }
  const hasGateCommit = (type: string, planId: string): boolean =>
    commitMessageLines.some((line) => line.startsWith(`${type}(${planId}):`));
  ...
  for (const planPath of tddPlanFiles) {
    ...
    const red = hasGateCommit('test', planId);
    const green = hasGateCommit('feat', planId);
    const refactor = hasGateCommit('refactor', planId);

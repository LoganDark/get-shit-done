...
function hasGenuineProjectMarker(dir: string): boolean {
  ...
  // VCS-audit fix (2026-07-08): a jj-native repo root is just as genuine a
  // project marker as `.git` — without this, an in-repo capability bundle
  // aliased via GSD_HOME in a `.planning`-less jj repo escaped the #1459
  // consent escalation (stayed trusted-global). Security-conservative
  // direction: more roots qualify → more consent demands.
  try {

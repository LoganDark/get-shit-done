...
// 19-12 next-merge port (AUDIT-01): VCS signals route through the ported
// adapter (backend auto-detect) — no raw child_process git. On jj repos the
// same signal shape is produced from jj primitives (status entries, bookmark
// remote-tracking ranges), so the front-door classifier works on jj-only
// checkouts. Field names keep the `git_*` envelope spelling — they are the
// public smart-entry JSON contract consumed by the /gsd workflow.
...
/** Read-only VCS signals. Any VCS error is swallowed → "no git signal". */
function readGitSignals(cwd: string): GitSignals {
  // 19-12 next-merge port (AUDIT-01): adapter-routed. createVcsAdapter throws
  // on a non-repo cwd — that IS the "is this a repo" probe (has_git=false).
  // 18-04 discipline: the dirty predicate keys on structured status entries[],
  // never on `.raw` (a clean jj working copy still emits human-readable text).
  let vcs: ReturnType<typeof createVcsAdapter>;
  try {
    vcs = createVcsAdapter(cwd);
  } catch {
    return { has_git: false, dirty: false, unpushed: false };
  }
  let dirty = false;
  try {
    dirty = vcs.status({ porcelain: true }).entries.length > 0;              // (was: status --porcelain)
  } catch { /* status failure → treat as clean */ }
  // Unpushed: commits on the current branch/bookmark not on its origin
  // remote-tracking ref (was: git log @{u}..HEAD --oneline; jj renders
  // `<bookmark>@origin..@`). Errors — no upstream, detached HEAD,
  // bookmark-less jj `@` — collapse to "not unpushed", same as upstream.
  let unpushed = false;
  try {
    const branch = vcs.refs.currentBookmarks()[0];
    if (branch) {
      unpushed = vcs.log({
        rev: expr.range(expr.remote(branch, 'origin'), expr.head()),
        maxCount: 1,
      }).length > 0;
    }
  } catch { /* no upstream / detached HEAD → not unpushed */ }

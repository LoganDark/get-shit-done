...
// 19-07 (AUDIT-01): worktree/workspace root detection routes through the
// ported VCS adapter so jj repos resolve correctly (see
// gitWorktreeInfoInternal below). The execGit seam above remains ONLY for
// the git-side base-branch precedence ladder (git-backend substrate — a jj
// repo takes the config-override tier or the 'main' default; bookmarks have
// no origin/HEAD analogue).
...
/**
 * Detect whether `cwd` sits inside a git worktree (or jj workspace), and if
 * so, return the absolute path of the worktree root.
 */

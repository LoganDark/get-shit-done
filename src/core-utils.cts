...
function detectSubRepos(cwd: string): string[] {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // without it, jj-backed sub-repos were invisible to auto-detection
      // (silent functionality loss; manual sub_repos config was the only
      // route). `.git` covers plain git AND colocated jj.
      const gitPath = path.join(cwd, entry.name, '.git');
      const jjPath = path.join(cwd, entry.name, '.jj');
      try {
        if (fs.existsSync(gitPath) || fs.existsSync(jjPath)) {

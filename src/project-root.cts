...
export function findProjectRoot(startDir: string): string {
  ...
  // VCS-audit fix (2026-07-08): also accept `.jj` so jj-native repos satisfy
  // the multi-repo / nested-project heuristics — a jj sub-repo under a
  // `.planning/` parent previously fell through to the weaker heuristics and
  // could resolve the wrong project root. Name kept for reference parity.
  function isInsideGitRepo(candidateParent: string): boolean {
    let d = resolvedStart;
    while (d !== fsRoot) {
      try {
        if (
          fs.existsSync(d + path.sep + '.git') ||
          fs.existsSync(d + path.sep + '.jj')
        ) return true;
      } catch {
        // ignore
      }

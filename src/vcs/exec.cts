 ...
 * Renamed `execGit` -> `execGitVcs` at port time (19-05) to remove the
 * naming-collision hazard with upstream's differently-shaped `execGit` in
 * src/shell-command-projection.cts (research naming-hazard mitigation).
 */
export function execGitVcs(cwd: string, args: string[], options: ExecOptions = {}): ExecResult {

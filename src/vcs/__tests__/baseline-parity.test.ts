 ...
 *
 * The test also asserts the underlying execGitVcs call (5-field shape) byte-equals
...
import { execGitVcs } from '../exec.cjs';
import { readWorktreeList as readWorktreePorcelain } from '../parse/worktree-list.cjs';
...
describe('GIT-02 byte-identity baselines (B-1)', () => {
  ...
  for (const f of files) {
    ...
    it(`${baseline.id} matches ${baseline.source}`, () => {
      ...
      try {
        // Direct execGitVcs (5-field) is the canonical GIT-02 equivalent.
        const got = execGitVcs(cwd, baseline.args);
        const norm = {
        ...
        if (
          ...
          // Resolve full HEAD sha via execGitVcs fallback for the lookup target,
          ...
          // The canonical execGitVcs call above already initialized the dir,
          ...
          // returns a 7+ hex string; the canonical execGitVcs call (above)
          ...
          const headRes = execGitVcs(cwd, ['rev-parse', 'HEAD']);
          const baseRes = execGitVcs(cwd, ['rev-parse', 'HEAD~3']);
          ...
          // flow + commitToSubrepo). The canonical execGitVcs call above
          ...
          // canonical execGitVcs call above already created the branch on this
          ...
          // execGitVcs call above already commits the staged path, so the
          ...
          // execGitVcs (a separate plumbing probe), wrap as expr.commit, and
          ...
          // Resolve baseArg/targetArg to full SHAs via execGitVcs so
          // expr.commit's shape validation accepts them.
          const baseRes = execGitVcs(cwd, ['rev-parse', baseArg]);
          const targetRes = execGitVcs(cwd, ['rev-parse', targetArg]);
          const baseSha = baseRes.stdout.trim();

 ...
 *
 * Phase 19 (19-11): the fork `gsd-sdk` binary is retired; the same
 * on-the-wire envelope contract now ships through the PORT-02 CLI bridge
 * (`gsd-core/bin/gsd-tools.cjs query <verb>` → src/vcs-command-router.cts).
 * This test invokes the checked-in gsd-tools binary via spawnSync against
 * a tmp git repo and inspects the actual on-the-wire JSON output.
 *
 * Pin: this is the only vitest file in the repo that runs the CLI binary
 * end-to-end. Do NOT replace with a programmatic import of the router —
 * the point is to exercise the exact dispatch path workflows hit at
 * run-time.
 */
...
beforeAll(() => {
	// Pre-flight: the checked-in CLI binary must exist.
	if (!existsSync(GSD_TOOLS)) {
		throw new Error(`gsd-sdk-binary-shape.integration: ${GSD_TOOLS} missing.`);
	}

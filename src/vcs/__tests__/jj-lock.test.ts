...
describe.skipIf(!jjAvailable)('acquireJjWriteLock — Phase 4 plan 03 (D-19)', () => {
	...
	beforeAll(() => {
		...
		// forks a child Node process that requires `gsd-core/bin/lib/vcs/jj/lock.cjs`.
		...
		// path is more robust.
		//
		// Phase 19 (19-11): fork `pnpm build:cjs` (sdk dist-cjs emit) replaced
		// by upstream's ADR-457 build (tsc → gsd-core/bin/lib).
	...
	// module (`gsd-core/bin/lib/vcs/jj/lock.cjs`), acquires + sleeps + releases.
	...
	it('concurrent acquire: second caller blocks until first releases (child-process)', async () => {
		...
		const cjsLockPath = join(process.cwd(), 'gsd-core/bin/lib/vcs/jj/lock.cjs');
		const child = spawn(
			'node',
			[
				'-e',

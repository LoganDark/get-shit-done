/**
 * vcs/exec unit tests.
 * Asserts the 5-field ExecResult shape, default-timeout binding, execGitVcs convenience,
...
import { vcsExec, execGitVcs, VcsExecError, DEFAULT_VCS_TIMEOUT_MS } from '../exec.cjs';
...
describe('execGitVcs', () => {
  it('runs `git --version` against a tmp git repo and returns exitCode 0 + stdout containing "git version"', () => {
    const result = execGitVcs(tmpDir, ['--version']);
    expect(result.exitCode).toBe(0);

...
import { readWorktreeList } from '../parse/worktree-list.cjs';
import {
  __vcsTestOnly,
  VcsIncompleteSubagentsError,
} from '../types.cjs';
import { readIncomplete } from '../jj/incomplete-work.cjs';
import { performGitParallelDispatch, performGitParallelFanIn, performGitParallelCancel } from '../git/parallel.cjs';
import { validateRefname } from '../refs-validator.cjs';
import type {
  GitVcsAdapter,
  CommitInput,
  CommitResult,
...
export function createGitAdapter(cwd: string): GitVcsAdapter {
  ...
  const commit = (input: CommitInput): CommitResult => {
    ...
    if (input.files && input.files.length > 0 && !input.respectStaged) {
      ...
      const headExists =
        execGitVcs(cwd, ['rev-parse', '--verify', '--quiet', 'HEAD']).exitCode === 0;
      const resetRes = execGitVcs(cwd, ['read-tree', headExists ? 'HEAD' : '--empty']);
      ...
      // guard.
      const addRes = execGitVcs(cwd, ['add', '-A', '--', ...input.files]);
    ...
    const commitRes = execGitVcs(cwd, args);
    ...
    const idRes = execGitVcs(cwd, ['rev-parse', 'HEAD']);
    return {
      exitCode: commitRes.exitCode,
      stdout: commitRes.stdout,
      stderr: commitRes.stderr,
  ...
  const status = (opts: StatusOpts = {}): StatusResult => {
    ...
    if (opts.porcelain === false) {
      const r = execGitVcs(targetCwd, ['status']);
  ...
  const diff = (opts: DiffOpts = {}): DiffResult => {
    ...
    if (opts.nameStatus) {
      ...
      const entries: import('../types.cjs').DiffNameStatusEntry[] = [];
      for (const line of r.stdout.split('\n')) {
        if (!line) continue;
        const cols = line.split('\t');
        if (cols.length < 2) continue;
        const statusRaw = cols[0];
        const letter = statusRaw[0] as import('../types.cjs').DiffNameStatusEntry['status'];
  ...
  const bookmarks = Object.freeze({
    list: (): Bookmark[] => {
      const r = execGitVcs(cwd, ['branch', '--format=%(refname:short)']);
      if (r.exitCode !== 0) return [];
    ...
    create: (name: string, rev: RevisionExpr, _opts?: { raw?: boolean }): void => {
      ...
      const r = execGitVcs(cwd, ['branch', '--', actualName, toGitRev(rev)]);
      if (r.exitCode !== 0) {
    ...
    move: (name: string, rev: RevisionExpr, _opts?: { raw?: boolean }): void => {
      ...
      const actualName = name;
      validateRefname(actualName);
      const r = execGitVcs(cwd, ['branch', '-f', '--', actualName, toGitRev(rev)]);
      if (r.exitCode !== 0) {
    ...
    delete: (name: string, opts?: { raw?: boolean; force?: boolean }): void => {
      ...
      const r = execGitVcs(cwd, ['branch', flag, '--', actualName]);
      if (r.exitCode !== 0) {
    ...
    exists: (name: string, _opts?: { raw?: boolean }): boolean => {
      ...
      const actualName = name;
      validateRefname(actualName);
      const r = execGitVcs(cwd, ['rev-parse', '--verify', '--quiet', actualName]);
      return r.exitCode === 0;
    },
  ...
  const currentBookmarks = (): string[] => {
    const r = execGitVcs(cwd, ['rev-parse', '--abbrev-ref', 'HEAD']);
    if (r.exitCode !== 0) return [];
  ...
  const currentBookmarksIn = (targetCwd: string): string[] => {
    const r = execGitVcs(targetCwd, ['rev-parse', '--abbrev-ref', 'HEAD']);
    if (r.exitCode !== 0) return [];
    const name = r.stdout.trim();
    if (!name || name === 'HEAD') return []; // detached
    return [name];
  ...
  const mergeBase = (a: RevisionExpr, b: RevisionExpr): string => {
    const r = execGitVcs(cwd, ['merge-base', toGitRev(a), toGitRev(b)]);
    if (r.exitCode !== 0) {
  ...
  const readBlob = (rev: RevisionExpr, blobPath: string): string => {
    const r = execGitVcs(cwd, ['show', `${toGitRev(rev)}:${blobPath}`]);
    if (r.exitCode !== 0) {
  ...
  const resolveShort = (rev: RevisionExpr): string => {
    const r = execGitVcs(cwd, ['rev-parse', '--short', toGitRev(rev)]);
    if (r.exitCode !== 0) {
  ...
  const countCommits = (opts: { rev?: RevisionExpr }): number => {
    ...
    const r = execGitVcs(cwd, ['rev-list', '--count', target]);
  ...
  const rootRevisions = (opts: { rev?: RevisionExpr }): string[] => {
    const target = opts.rev ? toGitRev(opts.rev) : 'HEAD';
    const r = execGitVcs(cwd, ['rev-list', '--max-parents=0', target]);
    if (r.exitCode !== 0) return [];
  ...
  const refExists = (rev: RevisionExpr): boolean => {
    ...
    const r = execGitVcs(cwd, ['cat-file', '-t', toGitRev(rev)]);
    return r.exitCode === 0;
  };
  ...
  const isIgnored = (p: string): boolean => {
    ...
    const r = execGitVcs(cwd, ['check-ignore', '-q', '--no-index', '--', p]);
    return r.exitCode === 0;
  };
  ...
  const workspace = Object.freeze({
    add: (input: WorkspaceAdd): WorkspaceInfo => {
      ...
      const head = execGitVcs(input.path, ['rev-parse', 'HEAD']);
      return {
        path: input.path,
    ...
    forget: (path: string): void => {
      const r = execGitVcs(cwd, ['worktree', 'remove', path]);
      if (r.exitCode !== 0) {
    ...
    context: () => {
      const top = execGitVcs(cwd, ['rev-parse', '--show-toplevel']);
      const gd = execGitVcs(cwd, ['rev-parse', '--git-dir']);
      const cd = execGitVcs(cwd, ['rev-parse', '--git-common-dir']);
    ...
    prune: (): ExecResult => execGitVcs(cwd, ['worktree', 'prune']),
    ...
    merge: (opts: WorkspaceMergeOpts): WorkspaceMergeResult => {
      ...
      const currentBranchRes = execGitVcs(cwd, ['rev-parse', '--abbrev-ref', 'HEAD']);
      const currentBranch =
      ...
      const mergeRes = execGitVcs(cwd, ['merge', '--no-ff', '-m', opts.message, branchRev]);
      ...
      const headRes = execGitVcs(cwd, ['rev-parse', 'HEAD']);
      ...
      if (opts.agentBookmark) {
        ...
        const delRes = execGitVcs(cwd, ['branch', '-D', '--', opts.agentBookmark]);
  ...
  const findConflicts = (opts: { scope: 'all' | 'working-copy' }): ConflictResult[] => {
    if (opts.scope === 'all') {
      ...
      const r = execGitVcs(cwd, ['ls-files', '--unmerged']);
    ...
    const r = execGitVcs(cwd, ['diff', '--check']);
    if (r.exitCode === 0) return [];
    const paths = new Set<string>();
    for (const line of r.stdout.split('\n')) {
  ...
  const gitOnly = Object.freeze({
    createAnnotatedTag: (name: string, message: string, rev: RevisionExpr): void => {
      const r = execGitVcs(cwd, ['tag', '-a', name, '-m', message, toGitRev(rev)]);
      if (r.exitCode !== 0) {
    ...
    revertAbort: (): ExecResult => {
      return execGitVcs(cwd, ['revert', '--abort']);
    },
    ...
    version: (): string => {
      ...
      const r = execGitVcs(cwd, ['--version']);
      if (r.exitCode !== 0) {
        throw new Error(
    ...
    configGet: (key: string): string | null => {
      ...
      const r = execGitVcs(cwd, ['config', '--get', key]);
    ...
    configSet: (key: string, value: string): void => {
      const r = execGitVcs(cwd, ['config', key, value]);
      if (r.exitCode !== 0) {
    ...
    gitDir: (): string => {
      const r = execGitVcs(cwd, ['rev-parse', '--git-dir']);
      if (r.exitCode !== 0) {
    ...
    gitCommonDir: (): string => {
      const r = execGitVcs(cwd, ['rev-parse', '--git-common-dir']);
      if (r.exitCode !== 0) {

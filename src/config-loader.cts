 ...
 *   - ./shell-command-projection.cjs (platformWriteSync, platformReadSync)
 *   - ./vcs/index.cjs        (createVcsAdapter — VCS probes, never raw git)
...
// from the fork's core.cjs reference, plan 02-11; re-homed here when upstream
// extracted config-loader from core.cts, epic #1267). `execGit` from
...
function isGitIgnored(cwd: string, targetPath: string): boolean {
  ...
  // VCS-audit fix (2026-07-08): the git-pinned probe below discovers the
  // repository by ancestor walk, so on a jj-NATIVE project (no `.git` at
  // cwd — colocated repos DO have one) `git check-ignore` would answer from
  // whatever unrelated ancestor git repo happens to contain the project —
  // an ancestor whose ignore rules could silently flip commit_docs/skip
  // decisions. Bound the probe to repos that actually carry git substrate
  // at cwd; jj-native keeps the documented deferred-degradation (false)
  // until the jj backend grows refs.isIgnored.

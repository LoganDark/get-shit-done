...
| 3097/3099 | `tests/bug-3097-3099-executor-worktree-path-safety.test.cjs` | Passes under jj-colocated; post-Phase-11 the test asserts gsd-executor.md and execute-phase.md reference `dispatch-cwd-safety.md` and that the literal `workspace.assert-dispatched-cwd` verb invocation is present in the safety reference. | carries-verbatim | Markdown-structural test: parses agent prompt files for the dispatched-cwd precondition guard. Phase 11 collapsed the former cwd-drift sentinel + absolute-path + HEAD-namespace guards into a single backend-opaque SDK verb call (`workspace.assert-dispatched-cwd`), so the assertion shape pivoted but the regression INTENT (defend against #3097/#3099) survives — the verb subsumes both failure modes by construction. | — (Phase 11 closed the jj-native translation gap by making the verb itself backend-opaque). |
...
- bug-3097 (Phase 4 multi-workspace audit, 2026-05-13; Phase 11 update, 2026-05-16): carries-verbatim — Phase 11 collapsed the four worktree-aware guards in gsd-executor.md's `<task_commit_protocol>` (cwd-drift sentinel, abs-path guard, HEAD-namespace regex, protected-ref deny-list) into a single `gsd-sdk query workspace.assert-dispatched-cwd --cwd .` call. The regression test was flipped accordingly (asserts the new verb literal + `dispatch-cwd-safety.md` reference). The verb is backend-opaque — its predicate over `vcs.workspace.list()` catches the same failure modes (cwd-drift, abs-path misrouting, HEAD on protected ref) by construction on both git and jj backends.

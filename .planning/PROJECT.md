...
**v1.3 in progress (started 2026-05-15).** v1.0 MVP shipped 2026-05-14 (8/8 phases). v1.1 first upstream sync shipped 2026-05-14 (1 phase, 5 plans). v1.2 (jujutsu is change-only — never commit id anywhere) shipped 2026-05-15 (1 phase, 3 plans, 14 reqs): `scripts/lint-vcs-no-commit-id.cjs` is the CI-blocking enforcer at 1038 files / 0 violations. v1.3 Phase 9 complete (2026-05-15, 5/5 plans): jj backend exposes `vcs.workspace.parallel.{dispatch,fanIn}` (composed over `octopus.ts` + `reap.ts`); reap classifier extended to `'merge-in-tree-conflict'` via the `conflicts()` revset; git-side stub throws `VcsNotImplementedError` pending Phase 10. v1.3 Phase 10 complete (2026-05-16, 6/6 plans, 5/5 must-haves re-verified after gap closure): git backend `vcs.workspace.parallel.{dispatch,fanIn}` ships in `sdk/src/vcs/git/parallel.ts`; cross-backend `FanInResult` shape uniform; STEP 1 gates on crashed-agent exit codes (no silent partial-commit merges); STEP 3 surplus sweep handle-scoped; TEST-13 contract suite green on both backends. v1.3 Phase 11 complete (orchestrator + agent rewire — raw-git dispatch blocks in `execute-phase.md`/`quick.md` deleted, `workspace.assert-dispatched-cwd` SDK verb shipped). v1.3 Phase 12 complete (2026-05-21, 3/3 plans, 5/5 must-haves): the A3 colocated pre-commit gap inherited from v1.0 Phase 4 is closed — the jj backend's `commit()` reliably fires the adapter-managed `.githooks/<stage>` hook in colocated mode (Path 1, with `GSD_HOOK_SKIP_COLOCATED` opt-out), guarded by a HOOK-07 fires-exactly-once regression test and an SC4 hook-idempotency audit. Phases 13 (CI parallel-path lane) and 14 (default flip + dogfood) remain.
...
---
*Last updated: 2026-05-21 after v1.3 Phase 12 (A3 colocated pre-commit fix) complete — the A3 gap inherited from v1.0 Phase 4 is closed: the jj backend's `commit()` reliably fires the adapter-managed `.githooks/<stage>` hook in colocated mode (Path 1, `GSD_HOOK_SKIP_COLOCATED` opt-out), guarded by a HOOK-07 fires-exactly-once regression test and an SC4 hook-idempotency audit. Phases 13 (CI parallel-path lane) and 14 (default flip + dogfood) remain. NOTE: `## Current State` and `### Validated` still carry pre-Phase-11 drift — a `/gsd:docs-update` reconciliation is recommended.*

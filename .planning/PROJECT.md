...
---
*Last updated: 2026-05-16 after v1.3 Phase 10 (git-side parallel verbs + classifier extension) complete — `vcs.workspace.parallel.{dispatch,fanIn}` ships on both backends via `sdk/src/vcs/git/parallel.ts` adapter-internal sidecar; cross-backend `FanInResult` shape uniform; crashed-agent gate + handle-scoped surplus sweep landed via gap-closure plans 10-05/10-06; TEST-13 contract suite green (8 runtime tests). Next gate: orchestrator rewire + `parallelization: true` default flip. PARALLEL-03/04 still dropped per 09-CONTEXT.md D-01/D-02.*

...
### Open Question Resolutions (mirrored from RESEARCH.md Open Questions section)

> Appended during planning. These do NOT renumber or reinterpret D-01..D-24; they record planner-adopted resolutions for the six Open Questions in 04-RESEARCH.md. Each plan that depends on a resolution cites the D-2x ID.

- **D-25 (WS-05 trigger predicate = "any subagent in any wave"):** First-subagent-dispatch in any wave triggers parent+merge slot creation. Single-subagent dispatch still gets a one-child octopus (cheap, forward-compat). Adopted by plan 05 createPhaseStructure / createSubagentHead. (Resolves RESEARCH Open Q1.)
- **D-26 (hooks.fire SDK query accepts `--cwd` with default `process.cwd()`):** The query bridge `gsd-sdk query hooks.fire <stage>` accepts an explicit `--cwd <path>` override; absent flag = process.cwd(). Phase 5 PROMPT-* workflow rewrites pass `--cwd=.` from the main workspace. Adopted by plan 06 sdk/src/query/hooks.ts. (Resolves RESEARCH Open Q2.)
- **D-27 (incomplete-work.md format = flat markdown, no frontmatter, no timestamps, one-per-phase):** The D-13 line shape `- {subagentName}: head={change_id_short}, workspace={path}, reason={reason}` is the entire format. Comments (`#` prefix) and blank lines tolerated. ONE file per phase regardless of nested-workspace depth. Adopted by plan 04 sdk/src/vcs/jj/incomplete-work.ts. (Resolves RESEARCH Open Q3.)
- **D-28 (30s lock-acquisition timeout default holds for Phase 4 v1):** Restatement of D-19 for traceability — no new metric or per-load tuning in Phase 4. Revisit during Phase 5 dogfood. (Resolves RESEARCH Open Q4.)
- **D-29 (workspace.reap always succeeds; the phase-merge gate is the queue file inside vcs.commit({phaseMergeFor})):** reap returns `ReapResult { abandoned, incomplete }` and does NOT throw on non-empty heads. The blocking happens later, inside `vcs.commit({phaseMergeFor})`, which reads the queue and throws `VcsIncompleteSubagentsError` if non-empty. Adopted by plan 04 performJjReap + jj.ts/git.ts commit() gate. (Resolves RESEARCH Open Q5.)
- **D-30 (octopus helper lives at sdk/src/vcs/jj/octopus.ts — UPSTREAM-02 sidecar):** Sidecar placement keeps the upstream-rebase surface zero-conflict. CJS bridge via `dist-cjs/vcs/jj/octopus.js` is produced automatically by the existing build pipeline; Phase 5 bin/lib/*.cjs consumers import through the bridge. Adopted by plan 05. (Resolves RESEARCH Open Q6.)

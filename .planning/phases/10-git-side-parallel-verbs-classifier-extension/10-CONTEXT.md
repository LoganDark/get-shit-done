...
- **D-12 [informational]:** **PROJECT.md target features bullet** ("Git backend implementation — new `parallel.*` verb bodies wrap the existing raw-git `worktree add` / `merge --no-ff` / `worktree remove` flow inside the adapter") — already accurate under D-01; no edit needed.

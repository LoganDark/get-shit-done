...
- [ ] **FLIP-04**: Invert PITFALL 1 doc at `sdk/src/vcs/backends/jj.ts:327` from negative-contract ("`LogEntry.hash` is `commit_id` NEVER `change_id`") to positive-contract ("`LogEntry.id` is the active backend's canonical revision identifier — `commit_id` on git, `change_id` on jj"). Update related JSDoc on the renamed `LogEntry.id` and `CommitResult.id` fields (per Phase 8 D-05 — no new ADR; the unified revision contract lives in `.planning/PROJECT.md` Key Decisions row + `.planning/REQUIREMENTS.md` core-value + the lint guard's failure message)
...
- [ ] **LINT-02**: Per-entry allowlist schema (`scripts/lint-vcs-no-commit-id.allow.json`) requires `reason` and `owner` fields per entry (per Phase 8 D-04 — `expires` is dropped; solo-dev context has no PR-review cadence to drive expires-based re-justification). Lint script CI-fails on missing required fields. Prevents Pitfall 7 (allowlist hollowing) via per-entry `reason` + `owner` + manual hygiene sweeps in the `$comment_2_1_09` style. Update existing `lint-vcs-no-raw-git.allow.json` schema to match for consistency (out-of-band tech-debt fold-in if cheap)
...
- [ ] **TEST-12**: Introduce `expect.extend({ toBeIdOf(received, kind) })` vitest custom matcher in `tests/__tools__/vitest-matchers.ts` (per Phase 8 D-02/D-02a — vitest custom matcher convention; the prior `expectIdShape` name was a placeholder for the actual API). Module augmentation lives at `tests/__tools__/vitest.d.ts` (both `Assertion` and `AsymmetricMatchersContaining` interfaces so it composes inside `expect.objectContaining({...})`). Registered via `setupFiles` in `sdk/vitest.config.ts` — loads once per vitest process. Sweep ~10-20 cross-backend `expect(gitResult).toEqual(jjResult)` and `.toMatch(/^[0-9a-f]/)` sites identified by AUDIT-03 to use the new matcher. Re-record golden-parity baselines via `tests/__tools__/capture-vcs-baselines.cjs` after FLIP-01..03 lands. Strict-green on both backend lanes; no skip-count regressions

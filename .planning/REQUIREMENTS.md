...
- [ ] **PARALLEL-01**: `vcs.workspace.parallel.dispatch(plan): ParallelDispatchHandle` ships on both backends. jj composes `octopus.createPhaseStructure` + N× `createSubagentSlot` under the new repo-scoped lock. git wraps `git worktree add` with internal serialization (Pitfall 5 — `.git/config.lock` race). Returns `{ phaseRoot, workspaces: [{ name, path, baseRev, agentId }], manifest }`.
- [ ] **PARALLEL-02**: `vcs.workspace.parallel.fanIn(handle, results): FanInResult` ships on both backends. jj uses one N-parent `jj new <p1>...<pN>` octopus form + batched `jj bookmark delete` under the new repo-scoped lock. git uses N-parent `git merge --no-ff <p1>...<pN>` + batched `git update-ref -d`. Returns `{ merged, conflicted, conflictedPaths, incompleteQueued, failedReaped, surplusBookmarks }` — same shape on both backends; `conflicted: boolean` distinguishes in-tree-conflict-success from crash (Pitfall 2).
- [ ] **PARALLEL-03**: Liveness-gated reap. `fanIn` probes every workspace for in-progress activity (jj op-log timestamp / git worktree mtime) before reaping; returns `{ partial: true, liveWorkspaces: [...] }` if any workspace is still being written to (Pitfall 3). `git worktree remove --force` is forbidden in the cross-backend path; non-force only.
- [ ] **PARALLEL-04**: New repo-scoped lock primitive `acquireJjRepoLock` (sentinel `.jj/repo/gsd-parallel-lock`) added as a sibling function in `sdk/src/vcs/jj/lock.ts` next to existing `acquireJjWriteLock`. Held across the entire dispatch→fanIn window (not per-workspace); distinct timeout characteristics. Cross-workspace contract test: `jj log -r 'divergent()' --no-graph` empty post-fan-in.
...
- [ ] **VCS-17**: New `sdk/src/vcs/jj/parallel.ts` composition layer. Imports `octopus.ts` + `reap.ts` + `workspace.merge` + the new `acquireJjRepoLock`. Consumed by `backends/jj.ts`.
...
- [ ] **TEST-14**: Cross-workspace `divergent()` revset assertion. Post-fan-in `jj log -r 'divergent()' --no-graph` MUST be empty on jj backend (PARALLEL-04 lock contract validation).

...
- **Operator manual actions** — ✅ squash done 2026-06-11: conflict resolutions squashed into merge change `vpzlrrlv` (no conflicted changes remain anywhere — jj won't push conflicts; `jj-vcs` synced to origin; the 19-01…19-13 fix/docs commits remain as ordinary history). ✅ reinstall done 2026-06-11 (`node bin/install.js --claude --global`) — close-day CR-01 + envelope fixes and the registry-update-check disable all propagated to `~/.claude`. Still open: optional `/gsd-secure-phase 19` (security enforcement enabled, no SECURITY.md).

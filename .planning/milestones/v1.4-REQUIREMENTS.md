# Requirements Archive: v1.4 Clean, consistent state for next upstream pull
...
- [x] **CLEANUP-02**: Orphan `.claude/jj-workspaces/phase-*-subagent-*` FS directories are reaped on `vcs.workspace.parallel.fan-in` success branch AND via `scripts/dogfood-restore.sh` post-restore cleanup; cross-backend test via `vcs-fixture.ts` Pattern B mkdtemp covers both jj-cell and git-cell paths (closes `v14-orphan-jj-workspace-dirs`)
...
| NAMING-01 | Phase 15 (plan 15.01) | Complete |
| VCS-21 | Phase 15 (plan 15.02) | Complete |
| VCS-22 | Phase 15 (plan 15.03) | Complete |
| PARALLEL-07 | Phase 15 (plan 15.04) | Complete |
| LINT-06 | Phase 16 (plan 16.01) | Complete |
| CLEANUP-02 | Phase 16 (plan 16.02) | Complete |
...
| CLEANUP-01 | Phase 18 (plan 18.01) | Deferred (v1.5+) |
| CLEANUP-03 | Phase 18 (plan 18.02) | Deferred (v1.5+) |
| CLEANUP-04 | Phase 18 (plan 18.02) | Deferred (v1.5+) |
| CLEANUP-05 | Phase 18 (plan 18.02) | Deferred (v1.5+) |
| CLEANUP-06 | Phase 18 (plan 18.02) | Deferred (v1.5+) |
| CLEANUP-07 | Phase 18 (plan 18.02) | Deferred (v1.5+) |
| TEST-17 | Phase 18 (plan 18.03) | Deferred (v1.5+) |
...
*Last updated: 2026-06-10 — Milestone close reconciliation: NAMING-01, VCS-21, VCS-22, PARALLEL-07 (Phase 15) and LINT-06, CLEANUP-02 (Phase 16) flipped to Complete per verified phase summaries (stale checkboxes). CLEANUP-01, CLEANUP-03..07, TEST-17 (Phase 18, never executed) marked Deferred (v1.5+) at milestone close — note their file references predate the Phase 19 upstream merge restructure (`sdk/` retired, `get-shit-done/` → `gsd-core/`) and need re-scoping. Phase 19 (upstream merge + fork-abstraction audit) executed under this milestone with no REQ-IDs of its own; see `.planning/phases/19-*/19-MERGE-AUDIT.md`.*
*Prior update: 2026-05-23 — Traceability table filled in during v1.4 roadmap creation (4 phases, 13 plans, 25 requirements mapped 100%, no orphans). Phase shape per research synthesis with one clarification: PARALLEL-07's plan extracts the `cleanupSubagentWorkspaces` shared helper as its Wave 1, consumed by Phase 16 CLEANUP-02 (single-owner per IP-5).*

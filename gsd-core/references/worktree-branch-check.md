...
> Ledgered git-only fragment — used by non-dispatched git-worktree flows only. The dispatched path is guarded cross-backend by `gsd_run query workspace.assert-dispatched-cwd` (see execute-phase.md).

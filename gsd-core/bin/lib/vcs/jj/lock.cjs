...
exports.acquireJjWriteLock = acquireJjWriteLock;
 ...
 */
function acquireJjWriteLock(workspacePath, opts = {}) {
    ...
    const sentinelDir = (0, node_path_1.join)(workspacePath, '.jj', 'working_copy');
    const sentinel = (0, node_path_1.join)(sentinelDir, 'gsd-lock');
    (0, node_fs_1.mkdirSync)(sentinelDir, { recursive: true });
    const deadline = Date.now() + timeout;
    let fd = null;
    while (fd === null) {
        try {
            // O_EXCL mode: 'wx' — fails with EEXIST if file already exists.
            fd = (0, node_fs_1.openSync)(sentinel, 'wx');
        }
        catch (err) {
            ...
            if (Date.now() >= deadline) {
                throw new Error(`acquireWriteLock timed out after ${timeout}ms: sentinel ${sentinel} still held`);
            }
    ...
    try {
        ...
        (0, exec_cjs_1.vcsExec)(workspacePath, 'jj', updateArgs);
    ...
    return {
        release: () => {
            ...
            if ((0, node_fs_1.existsSync)(sentinel)) {
                try {
                    (0, node_fs_1.unlinkSync)(sentinel);
                }
                catch { /* sentinel already gone */ }
            }
        },
    };
}

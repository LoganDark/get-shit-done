...
exports.WORKSPACE_NAME_RE = void 0;
exports.cleanupSubagentWorkspaces = cleanupSubagentWorkspaces;
 ...
 */
exports.WORKSPACE_NAME_RE = /^phase-(\d+)-subagent-\d+$/;
/**
 ...
 */
function cleanupSubagentWorkspaces(mainRepoRoot, phaseNumber, workspaces) {
    ...
    if (workspaces && workspaces.length > 0) {
        ...
        const workspacesDir = (0, node_path_1.join)(mainRepoRoot, '.claude', 'jj-workspaces');
        ...
        iterList = (0, node_fs_1.readdirSync)(workspacesDir)
            .filter((d) => pattern.test(d))
            .map((d) => ({ name: d, path: (0, node_path_1.join)(workspacesDir, d) }));
    }
    const abandoned = [];
    const failedReaped = [];
    for (const ws of iterList) {
        ...
        void (0, exec_cjs_1.vcsExec)(mainRepoRoot, 'jj', forgetArgs);
        ...
        if ((0, node_fs_1.existsSync)(ws.path)) {
            try {
                (0, node_fs_1.rmSync)(ws.path, { recursive: true, force: true });

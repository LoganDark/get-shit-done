...
exports.parseDiffCheckPath = parseDiffCheckPath;
exports.createGitAdapter = createGitAdapter;
const node_child_process_1 = require("node:child_process");
const node_path_1 = require("node:path");
const exec_cjs_1 = require("../exec.cjs");
const expr_cjs_1 = require("../expr.cjs");
const git_rev_cjs_1 = require("../parse/git-rev.cjs");
const worktree_list_cjs_1 = require("../parse/worktree-list.cjs");
const types_cjs_1 = require("../types.cjs");
const incomplete_work_cjs_1 = require("../jj/incomplete-work.cjs");
const parallel_cjs_1 = require("../git/parallel.cjs");
...
function parseDiffCheckPath(line) {
    // Format: <path>:<line>[:<col>]: <description>
    const m = line.match(/^(.+?):\d+(?::\d+)?:\s/);
    return m && m[1] ? m[1] : null;
}
// ─── Factory ────────────────────────────────────────────────────────────────
function createGitAdapter(cwd) {
    // ─── commit ──────────────────────────────────────────────────────────────
    /**
    ...
    const commit = (input) => {
        ...
        if (input.files !== undefined && input.files.length === 0) {
            throw new Error('commit({files:[]}) is ambiguous; pass files: undefined for `git commit -am`, ' +
        ...
        if (input.files && input.files.length > 0 && !input.respectStaged) {
            ...
            const headExists = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--verify', '--quiet', 'HEAD']).exitCode === 0;
            const resetRes = (0, exec_cjs_1.execGitVcs)(cwd, ['read-tree', headExists ? 'HEAD' : '--empty']);
            ...
            // guard.
            const addRes = (0, exec_cjs_1.execGitVcs)(cwd, ['add', '-A', '--', ...input.files]);
        ...
        const commitRes = (0, exec_cjs_1.execGitVcs)(cwd, args);
        ...
        const idRes = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', 'HEAD']);
        return {
            exitCode: commitRes.exitCode,
            stdout: commitRes.stdout,
            stderr: commitRes.stderr,
    ...
    const log = (opts = {}) => {
        ...
        if (opts.allRefs)
            args.push('--all');
        if (opts.rev)
            args.push((0, git_rev_cjs_1.toGitRev)(opts.rev));
        if (opts.paths && opts.paths.length > 0)
            args.push('--', ...opts.paths);
        const r = (0, exec_cjs_1.execGitVcs)(cwd, args);
        if (r.exitCode !== 0)
            return [];
        // `-z` uses NUL between records. r.stdout is trimmed by execGitVcs, so the
    ...
    //
    // Phase 2.1 #3061: execGitVcs's stdout.trim() strips the leading space when the
    ...
    const status = (opts = {}) => {
        ...
        if (opts.porcelain === false) {
            const r = (0, exec_cjs_1.execGitVcs)(targetCwd, ['status']);
        ...
        const statusTrimEnd = (gitArgs) => {
            const r = (0, node_child_process_1.spawnSync)('git', gitArgs, {
                ...
                stdio: 'pipe',
                encoding: 'utf-8',
                timeout: exec_cjs_1.DEFAULT_VCS_TIMEOUT_MS,
                windowsHide: true,
            });
    ...
    const bookmarks = Object.freeze({
        list: () => {
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['branch', '--format=%(refname:short)']);
            if (r.exitCode !== 0)
                return [];
        ...
        create: (name, rev, _opts) => {
            ...
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['branch', '--', actualName, (0, git_rev_cjs_1.toGitRev)(rev)]);
            if (r.exitCode !== 0) {
        ...
        move: (name, rev, _opts) => {
            ...
            const actualName = name;
            (0, refs_validator_cjs_1.validateRefname)(actualName);
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['branch', '-f', '--', actualName, (0, git_rev_cjs_1.toGitRev)(rev)]);
            if (r.exitCode !== 0) {
        ...
        delete: (name, opts) => {
            ...
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['branch', flag, '--', actualName]);
            if (r.exitCode !== 0) {
        ...
        exists: (name, _opts) => {
            ...
            const actualName = name;
            (0, refs_validator_cjs_1.validateRefname)(actualName);
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--verify', '--quiet', actualName]);
            return r.exitCode === 0;
        },
    ...
    // (call execGitVcs, parse the standard 5-field result, return typed scalar).
    ...
    const currentBookmarks = () => {
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--abbrev-ref', 'HEAD']);
        if (r.exitCode !== 0)
            return [];
    ...
    // to execGitVcs as the spawned-process cwd; `git rev-parse --abbrev-ref HEAD`
    // resolves against that workspace's HEAD.
    const currentBookmarksIn = (targetCwd) => {
        const r = (0, exec_cjs_1.execGitVcs)(targetCwd, ['rev-parse', '--abbrev-ref', 'HEAD']);
        if (r.exitCode !== 0)
            return [];
        const name = r.stdout.trim();
        if (!name || name === 'HEAD')
    ...
    const mergeBase = (a, b) => {
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['merge-base', (0, git_rev_cjs_1.toGitRev)(a), (0, git_rev_cjs_1.toGitRev)(b)]);
        if (r.exitCode !== 0) {
            throw new exec_cjs_1.VcsExecError(`refs.mergeBase failed: ${r.stderr || r.stdout}`, {
                ...
                stderr: r.stderr,
                timedOut: false,
                args: ['merge-base', (0, git_rev_cjs_1.toGitRev)(a), (0, git_rev_cjs_1.toGitRev)(b)],
            });
        }
    ...
    const readBlob = (rev, blobPath) => {
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['show', `${(0, git_rev_cjs_1.toGitRev)(rev)}:${blobPath}`]);
        if (r.exitCode !== 0) {
            throw new exec_cjs_1.VcsExecError(`refs.readBlob failed: ${r.stderr || r.stdout}`, {
                ...
                stderr: r.stderr,
                timedOut: false,
                args: ['show', `${(0, git_rev_cjs_1.toGitRev)(rev)}:${blobPath}`],
            });
        }
        return r.stdout;
    };
    const resolveShort = (rev) => {
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--short', (0, git_rev_cjs_1.toGitRev)(rev)]);
        if (r.exitCode !== 0) {
            throw new Error(`refs.resolveShort failed: ${r.stderr || r.stdout}`);
        }
        return r.stdout.trim();
    };
    const countCommits = (opts) => {
        const target = opts.rev ? (0, git_rev_cjs_1.toGitRev)(opts.rev) : 'HEAD';
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-list', '--count', target]);
        if (r.exitCode !== 0)
            return 0;
    ...
    const rootRevisions = (opts) => {
        const target = opts.rev ? (0, git_rev_cjs_1.toGitRev)(opts.rev) : 'HEAD';
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-list', '--max-parents=0', target]);
        if (r.exitCode !== 0)
            return [];
    ...
    const matchPrefix = (id, prefix) => {
        ...
        const rawId = (0, git_rev_cjs_1.toGitRev)(id);
    ...
    const refExists = (rev) => {
        ...
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['cat-file', '-t', (0, git_rev_cjs_1.toGitRev)(rev)]);
        return r.exitCode === 0;
    };
    const isIgnored = (p) => {
    ...
    const workspace = Object.freeze({
        add: (input) => {
            ...
            if (input.name)
                (0, refs_validator_cjs_1.validateRefname)(input.name);
            if (input.path.startsWith('-')) {
                throw new Error(`workspace.add: invalid path '${input.path}' (leading '-')`);
            }
            const baseRevArg = input.baseRef ? [(0, git_rev_cjs_1.toGitRev)(input.baseRef)] : [];
            const branchArg = input.name ? ['-b', input.name] : [];
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['worktree', 'add', ...branchArg, input.path, ...baseRevArg]);
            if (r.exitCode !== 0) {
                throw new Error(`workspace.add failed: ${r.stderr || r.stdout}`);
            }
            const head = (0, exec_cjs_1.execGitVcs)(input.path, ['rev-parse', 'HEAD']);
            return {
                path: input.path,
        ...
        forget: (path) => {
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['worktree', 'remove', path]);
            if (r.exitCode !== 0) {
        ...
        list: () => {
            ...
            const result = (0, worktree_list_cjs_1.readWorktreeList)(cwd);
            if (!result.ok)
                return [];
            return result.entries.map((e) => ({ path: e.path, rev: e.head, locked: e.locked }));
        },
        ...
        context: () => {
            const top = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--show-toplevel']);
            const gd = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--git-dir']);
            const cd = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--git-common-dir']);
            ...
            if (top.exitCode !== 0 || gd.exitCode !== 0 || cd.exitCode !== 0) {
                throw new Error(`workspace.context: not a git repo at ${cwd}: ${top.stderr || gd.stderr || cd.stderr}`);
            }
            const effectiveRoot = (0, node_path_1.resolve)(cwd, top.stdout.trim());
            const gitDirResolved = (0, node_path_1.resolve)(cwd, gd.stdout.trim());
            const gitCommonDirResolved = (0, node_path_1.resolve)(cwd, cd.stdout.trim());
        ...
        prune: () => (0, exec_cjs_1.execGitVcs)(cwd, ['worktree', 'prune']),
        reap: (opts) => {
            ...
            for (const entry of entries) {
                ...
                if (!name.startsWith(opts.phaseNamePrefix))
                    continue;
                const removeRes = (0, exec_cjs_1.execGitVcs)(cwd, ['worktree', 'remove', entry.path]);
                if (removeRes.exitCode !== 0) {
                    throw new Error(`git workspace.reap remove failed for ${entry.path}: ${removeRes.stderr || removeRes.stdout}`);
                }
        ...
        merge: (opts) => {
            ...
            const currentBranchRes = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--abbrev-ref', 'HEAD']);
            const currentBranch = currentBranchRes.exitCode === 0 ? currentBranchRes.stdout.trim() : '';
            if (currentBranch !== opts.mainBookmark) {
                throw new exec_cjs_1.VcsExecError(`workspace.merge: mainBookmark must match current branch on git backend (got mainBookmark='${opts.mainBookmark}', HEAD on '${currentBranch}')`, {
                    exitCode: 1,
                    stdout: '',
                    stderr: '',
                    timedOut: false,
            ...
            const branchRev = (0, git_rev_cjs_1.toGitRev)(opts.branch);
            const mergeRes = (0, exec_cjs_1.execGitVcs)(cwd, ['merge', '--no-ff', '-m', opts.message, branchRev]);
            ...
            const headRes = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', 'HEAD']);
            ...
            if (opts.agentBookmark) {
                ...
                const delRes = (0, exec_cjs_1.execGitVcs)(cwd, ['branch', '-D', '--', opts.agentBookmark]);
        ...
        parallel: Object.freeze({
            dispatch: (opts) => (0, parallel_cjs_1.performGitParallelDispatch)({ mainRepoRoot: cwd, vcs: { workspace }, ...opts }),
            fanIn: (handle, results) => (0, parallel_cjs_1.performGitParallelFanIn)(cwd, handle, results),
            cancel: (handle) => (0, parallel_cjs_1.performGitParallelCancel)(cwd, handle),
        }),
    });
    ...
    const acquireWriteLock = (_workspace, _opts) => {
        return { release: () => { } };
    };
    ...
    const findConflicts = (opts) => {
        if (opts.scope === 'all') {
            ...
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['ls-files', '--unmerged']);
        ...
        const r = (0, exec_cjs_1.execGitVcs)(cwd, ['diff', '--check']);
        if (r.exitCode === 0)
            return [];
        const paths = new Set();
        for (const line of r.stdout.split('\n')) {
    ...
    const push = (opts = {}) => {
        ...
        if (opts.ref)
            args.push((0, git_rev_cjs_1.toGitRev)(opts.ref));
        return (0, exec_cjs_1.execGitVcs)(cwd, args);
    };
    const fetch = (opts = {}) => {
        const args = ['fetch'];
        if (opts.remote)
    ...
    const gitOnly = Object.freeze({
        createAnnotatedTag: (name, message, rev) => {
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['tag', '-a', name, '-m', message, (0, git_rev_cjs_1.toGitRev)(rev)]);
            if (r.exitCode !== 0) {
        ...
        // array (no shell string), matching every other execGitVcs call in this file.
        revert: (opts) => {
            const args = ['revert'];
            if (opts.noCommit)
                args.push('--no-commit');
        ...
        revertAbort: () => {
            return (0, exec_cjs_1.execGitVcs)(cwd, ['revert', '--abort']);
        },
        ...
        version: () => {
            ...
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['--version']);
            if (r.exitCode !== 0) {
        ...
        // shapes when needed.
        init: () => {
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['init']);
            if (r.exitCode !== 0) {
        ...
        configGet: (key) => {
            ...
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['config', '--get', key]);
            if (r.exitCode === 0)
                return r.stdout.trim();
            if (r.exitCode === 1)
                return null;
        ...
        configSet: (key, value) => {
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['config', key, value]);
            if (r.exitCode !== 0) {
        ...
        gitDir: () => {
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--git-dir']);
            if (r.exitCode !== 0) {
                throw new Error(`gitOnly.gitDir failed: ${r.stderr || r.stdout}`);
            }
            return (0, node_path_1.resolve)(cwd, r.stdout.trim());
        },
        gitCommonDir: () => {
            const r = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', '--git-common-dir']);
            if (r.exitCode !== 0) {
    ...
    const testOnly = Object.freeze({
        snapshot: () => {
            const head = (0, exec_cjs_1.execGitVcs)(cwd, ['rev-parse', 'HEAD']);
            if (head.exitCode !== 0) {
                throw new Error(`__vcsTestOnly.snapshot: rev-parse HEAD failed: ${head.stderr}`);
            }
            (0, exec_cjs_1.execGitVcs)(cwd, ['update-ref', 'refs/gsd/test-snapshot', head.stdout]);
            return { id: head.stdout, kind: 'git' };
        },
        restore: (handle) => {
            const reset = (0, exec_cjs_1.execGitVcs)(cwd, ['reset', '--hard', handle.id]);
            if (reset.exitCode !== 0) {
                throw new Error(`__vcsTestOnly.restore: reset failed: ${reset.stderr}`);
            }
            (0, exec_cjs_1.execGitVcs)(cwd, ['clean', '-fdx']);
        },
    });

...
function commitIdOf(cwd, changeId) {
    ...
    if (r.exitCode !== 0) {
        throw new exec_cjs_1.VcsExecError(`jj-id.commitIdOf failed for change ${changeId}: ${r.stderr || r.stdout}`, {
            exitCode: r.exitCode,
            stdout: r.stdout,
            stderr: r.stderr,
            timedOut: r.timedOut,
...
function changeIdOf(cwd, commitId) {
    ...
    const r = (0, exec_cjs_1.vcsExec)(cwd, 'jj', args);
    if (r.exitCode !== 0) {
        throw new exec_cjs_1.VcsExecError(`jj-id.changeIdOf failed for commit ${commitId}: ${r.stderr || r.stdout}`, {
            exitCode: r.exitCode,
            stdout: r.stdout,
            stderr: r.stderr,
            timedOut: r.timedOut,

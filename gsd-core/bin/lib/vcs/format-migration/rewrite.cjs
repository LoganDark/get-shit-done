...
exports.COMMIT_KEY_ALLOWLIST = exports.JJ_CID_RE = exports.GIT_SHA_RE = void 0;
exports.migrateContent = migrateContent;
exports.findEligibleZones = findEligibleZones;
/**
 ...
 */
exports.GIT_SHA_RE = /(?<![0-9a-fA-F])([0-9a-f]{7,40})(?![0-9a-fA-F])/g;
/**
 ...
 * (or vice versa).
 */
exports.JJ_CID_RE = /(?<![k-z])([k-z]{8,12})(?![k-z])/g;
/**
 ...
 */
exports.COMMIT_KEY_ALLOWLIST = new Set([
    'resolution_commit',
    'commit',
    'commit_hash',
    'commit_id',
 ...
 */
function migrateContent(content, direction, resolve, filePath) {
    const re = direction === 'git→jj' ? exports.GIT_SHA_RE : exports.JJ_CID_RE;
    ...
    while ((m = re.exec(content)) !== null) {
        ...
        switch (r.kind) {
            ...
            case 'ancestor': {
                ...
                out += `${r.targetId}\`[was ${breadcrumbLabel}:${id}]\``;
                orphans.push({
                    original: id,
                    resolved: r.targetId,
...
function findEligibleZones(content) {
    ...
    const isFencedLine = (i) => fencedLineRanges.some(([s, e]) => i >= s && i <= e);
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isInFrontmatter && !isFrontmatterDelim) {
            ...
            if (kv !== null) {
                ...
                const value = kv[2];
                if (exports.COMMIT_KEY_ALLOWLIST.has(key) && value.length > 0) {
 ...
 */
function isMatchInEligibleZone(matchStart, matchEnd, zones) {
    for (const z of zones) {
        if (matchStart >= z.start && matchEnd <= z.end)
            return true;
    }
    return false;
}

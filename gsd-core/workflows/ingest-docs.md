...
- If `has_git: false` and `MODE=new`: initialize git — but ONLY when the project is not jj-native (VCS-audit 2026-07-08: `has_git: false` is also what a non-colocated jj repo reports; a stray `git init` there would flip backend detection toward git):

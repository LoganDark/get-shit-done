...
**If branches exist — probe the backend first (VCS-audit 2026-07-08):** the
Squash/History merge options below are git-mode-only (the `git checkout`
base-branch hop is TODO 05-05, and `query merge`/`query reset` return typed
refusals on jj) — offering them on jj produced failed checkouts followed by
bogus merge-less commits.

```bash
_VCS_KIND=$(jq -r '.vcs.adapter // empty' .planning/config.json 2>/dev/null)
if [ -z "$_VCS_KIND" ] || [ "$_VCS_KIND" = "auto" ]; then
  if [ -d .git ] || [ -f .git ]; then _VCS_KIND=git; elif [ -d .jj ]; then _VCS_KIND=jj; else _VCS_KIND=git; fi
fi
```

**If `_VCS_KIND` = jj:** bookmarks already serve as merge markers on jj (the
orchestrator advanced `gsd/phase-*`; there is no branch-hop to replay).
Present only **Delete without merging** / **Keep branches** via
AskUserQuestion, and for deletion guide the user to `jj bookmark forget
<name>` instead of the git block below. Then continue to git_tag.

...
```bash
# TODO(05-05 sweep): no remote-prune/branch-delete-shaped query verbs yet — stale-branch pruning stays git-mode; jj users prune bookmarks via jj bookmark forget.
if [ -d .git ] || [ -f .git ]; then
  git fetch --prune 2>/dev/null || true
else
  echo "Stale-branch pruning is git-only; on jj use: jj bookmark list / jj bookmark forget <name>"
fi
```

Then enumerate candidates (protected branch names are excluded even if their upstream is gone; git-mode only, same gate):

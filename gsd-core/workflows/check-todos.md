...
```bash
# Move is captured by the commit below (--allow-deletions records the pending/ deletion; jj auto-tracks).
gsd_run query commit "docs: start work on todo - [title]" --files .planning/todos/pending/[filename] .planning/todos/completed/[filename] .planning/STATE.md --allow-deletions
```

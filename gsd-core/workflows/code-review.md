...
if [ -n "$FILES_OVERRIDE" ]; then
  REVIEW_FILES=()
  # TODO(05-05 sweep): `gsd_run query head-ref` does not yet expose `--show-toplevel`;
  ...
  for file_path in "${FILES_ARRAY[@]}"; do
    ...
    # agents/gsd-executor.md — admit ONLY $REPO_ROOT itself OR strict
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  # filter client-side on the structured `.subject` field. LogEntry.id is the unified
  # revision id (hex on git, [k-z] change-id on jj).
  PHASE_COMMITS=$(gsd_run query log --all --max-count 500 \
    | jq -r ".entries[] | select(.subject | test(\"\\\(${PADDED_PHASE}\\\)|\\\(${PADDED_PHASE}-\")) | .id" 2>/dev/null)
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    # TODO(05-05 sweep): no rev-parse-shaped query verb yet — head-ref covers HEAD but not arbitrary revs.
    if ! git rev-parse "${DIFF_BASE}" >/dev/null 2>&1; then
      DIFF_BASE=$(echo "$PHASE_COMMITS" | tail -1)
    fi

    # Run diff via the query verb with specific exclusions (per D-03). The diff verb returns
    # structured name-only output; exclusion patterns are applied client-side
    # because `gsd_run query diff` does not yet expose pathspec-exclude pass-through.
    DIFF_FILES=$(gsd_run query diff --name-only --range "${DIFF_BASE}..HEAD" \
    ...
    echo "File scope: ${#REVIEW_FILES[@]} files from gsd_run query diff (base: ${DIFF_BASE})"
  else
...
if [ \"$FALLOW_SCOPE\" = \"phase\" ]; then
  # git-substrate (fork ledger): fallow's --changed-since consumes a git rev and
  # fallow itself shells git internally — this scoping probe is only meaningful
  # where fallow can run (a git checkout). On jj-only repos the probe emits
  # nothing (2>/dev/null) and the block degrades to fallow's repo-scope
  # auto-detection, which is the documented fallback below.
...
```bash
# Same query-routed log + client-side subject filter as compute_file_scope.
PHASE_COMMITS=$(gsd_run query log --all --max-count 500 \
  | jq -r ".entries[] | select(.subject | test(\"\\\(${PADDED_PHASE}\\\)|\\\(${PADDED_PHASE}-\")) | .id" 2>/dev/null)
if [ -n "$PHASE_COMMITS" ]; then
  DIFF_BASE=$(echo "$PHASE_COMMITS" | tail -1)^

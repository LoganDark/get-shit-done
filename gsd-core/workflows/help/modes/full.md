...
**GSD Core** (Get Shit Done) creates hierarchical project plans optimized for solo agentic development with Claude Code.
...
- **`/gsd:undo --last N | --phase NN | --plan NN-MM`** — Safe revert. Roll back phase or plan commits using the phase manifest with dependency checks. On jj add `--force` to override shared-history protection (passes `--ignore-immutable`).

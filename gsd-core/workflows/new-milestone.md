...
Commit the phase archive move + source removal together with the milestone start (atomic — no orphaned uncommitted deletions, no un-archived dirs carried forward). `phases.clear` archives each non-999 dir to `milestones/<version>-phases/`; naming both dirs in `--files` captures the new archive and the removals together (#1871). The adapter's commit verb captures WC state for the named paths on both backends — a separate pre-staging `git add` would be wiped by the verb's index reset (#3061 semantics), so the paths ride the commit call itself; `--allow-deletions` covers the removed source dirs.

```bash
gsd_run query commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md .planning/milestones/ .planning/phases/ --allow-deletions
```

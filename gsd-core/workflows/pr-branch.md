...
<backend_gate>
Run this gate before any step below — the whole workflow is git-mode.

```bash
# TODO(05-05 sweep): pr-branch is git-only — the history filtering below is built on git cherry-pick, which has no jj analog yet; on jj, build the PR branch with jj duplicate/rebase onto the base bookmark, then jj git push --bookmark <name>.
...
if [ "$_VCS_KIND" = "jj" ]; then
  echo "pr-branch is git-only (cherry-pick history filtering — TODO(05-05 sweep): no jj analog)."
  echo "On jj: build the PR branch with jj duplicate/rebase onto the base bookmark, then push it with jj's git-push (--bookmark)."
  exit 0
fi
```
</backend_gate>

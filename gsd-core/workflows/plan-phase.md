...
if [ -n "$DIRTY_PATHS" ]; then
	# Categorize so the operator can diagnose which class of leak fired.
...
**Why unconditional and not just `.planning/`:** the gate's job is to catch ALL forms of "we're declaring done but state isn't durable" — orchestrator mutations, subagent artifacts, mixed-in WIP. Restricting to `.planning/` would only catch case (a)/(b) and silently rubber-stamp cases (c)/(d). The categorization in the error message keeps the diagnostic story clean without weakening the gate.

**Auto-advance path:** When §15 dispatches execute-phase, execute-phase has its OWN `assert_clean_wc` gate before its "PHASE COMPLETE" emission. This gate here protects the manual route only (where §15 routes to `<offer_next>`).

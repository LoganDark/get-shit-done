...
Resolve the backend first — the tag/log stats methods below are git-mode only:

```bash
# Release stats are tag-based (git-mode); jj milestones use gsd/milestone/* bookmarks (REFS-06) — stats gathering TODO(05-05 sweep).
...
If `[ "$_VCS_KIND" = "git" ]` is false (jj backend), skip Methods 1–3 and fall straight through to Method 4 (skip stats).
...
**Method 4 — Skip stats** (if none of the above work, or on the jj backend):

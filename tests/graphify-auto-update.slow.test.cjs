...
describe('auto-update', () => {
  ...
  describe('hook — dispatch path (all gates pass)',
    ...
    test('writes status file with status=running synchronously before returning', async (t) => {
      ...
      assert.ok(/^[0-9a-f]{7,40}$/.test(status.head_at_build), 'head_at_build must be a commit sha'); // vcs-lint:allow-commit-id-here git-substrate hook output asserted inside a git-only fixture (gsd-graphify-update.sh is ledgered read-only-git-probes-with-gated-no-op-on-jj)

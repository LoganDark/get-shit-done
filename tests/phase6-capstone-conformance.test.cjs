...
describe('ADR-857 Phase 6 capstone conformance (#1139)', () => {
  ...
  test('host loop bodies are materially smaller than the pre-phase-6 baseline (#1168)', () => {
    ...
    // jj-fork adjustment: the fork's Phase 18 assert_clean_wc gates + Phase 11
    // fan-in envelope machinery legitimately add ~4-6 KB to both orchestrators
    // (extraction blocked — ~50 tests pin those blocks in-file). Baselines are
    // the upstream pre-phase-6 numbers plus that ledgered fork delta, so the
    // ADR-857 shrink assertion still binds against fork-equivalent baselines.
    // VCS-audit 2026-07-08: execute-phase +612 B for the cross-backend #48
    // guard (dual root ladder + jj dispatched-workspace refusal) and the
    // enforced branching-block substrate gate — privileged host machinery,
    // same class as the worktree record-agent ledger above.
    const PRE_PHASE6 = { 'plan-phase.md': 98519, 'execute-phase.md': 101070 };
    const notShrunk = [];

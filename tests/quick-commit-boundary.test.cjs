...
describe('quick.md pre-dispatch PLAN.md commit (#2432)', () => {
  ...
  test('#1265 executor materializes PLAN.md from the plan commit when worktree starts at parent', () => {
    ...
    // 19-12 re-point: the jj fork materializes through the adapter bridge —
    // `gsd_run query read-blob --rev "${QUICK_PLAN_COMMIT}" --path … --pick content`
    // (git: `git show <rev>:<path>`; jj: `jj file show -r <rev> <path>`).
    assert.ok(
      executorPrompt.includes('query read-blob') && executorPrompt.includes('QUICK_PLAN_COMMIT'),
      'executor prompt must materialize PLAN.md from the plan commit via query read-blob when absent (#1265; 19-12 re-point)'
    );
    assert.ok(
      executorPrompt.indexOf('query read-blob') < executorPrompt.indexOf('<files_to_read>'),

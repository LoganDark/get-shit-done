...
// ─── Shared timeout stubs (Plan 02-04 Task 2 retarget) ──────────────────────

/**
 * Plan 02-04 Task 2 (D-06): worktree-safety.cjs migrated from raw execGit to
 * vcs.workspace.{context,prune} + a deps.readPorcelain seam for the porcelain
 * reader. The bug-3281 timeout-simulation tests retarget onto these new seams:
 *
 *   - deps.readPorcelain: simulates `readWorktreeList` returning the
 *     {ok:false, reason:'git_timed_out'} shape, which is the exact thing the
 *     SDK porcelain parser would return on a timeout. Mirrors the previous
 *     execGit-timed-out injection on the line-80 path.
 *
 *   - deps.vcs: a minimal fake adapter whose workspace.{context,prune} either
 *     throw (context — prior raw behavior was exit≠0 → not_git_repo) or return
 *     a timedOut ExecResult (prune — preserves AC4's first-class timedOut field).
 */
function makeReadPorcelainTimedOut() {
  return function stubReadPorcelainTimedOut(_repoRoot) {
    return {
      ok: false,
...
function makeFakeVcsTimedOut() {
  // Phase 2 D-06 seam: a fake VcsAdapter whose workspace.prune() reports the
  // timed-out ExecResult shape. workspace.context() throws (the production
  // adapter throws on rev-parse failure incl. timeout — see git.ts:396-399).
  return Object.freeze({
    kind: 'git',
    workspace: Object.freeze({
      context: () => {
        const e = new Error('workspace.context: not a git repo: timed out');
        throw e;
      },
      prune: () => ({
        exitCode: null,
...
describe('bug-3281 AC1: worktree functions return degraded-ok on timeout, not throw', () => {
  test('planWorktreePrune returns action=skip when execGit times out', () => {
    ...
    let result;
    try {
      result = planWorktreePrune('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });
    } catch {
      threw = true;
    }
  ...
  test('executeWorktreePrunePlan returns ok:false when plan is skip (timeout path)', () => {
    ...
    const plan = planWorktreePrune('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });
    const result = executeWorktreePrunePlan(plan, { vcs: makeFakeVcsTimedOut() });
  ...
  test('inspectWorktreeHealth returns ok:false when git times out', () => {
    ...
    let result;
    try {
      result = inspectWorktreeHealth('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });
    } catch {
      threw = true;
    }
  ...
  test('listLinkedWorktreePaths returns ok:false on timeout, not throw', () => {
    ...
    let result;
    try {
      result = listLinkedWorktreePaths('/tmp', { readPorcelain: makeReadPorcelainTimedOut() });
    } catch {
      threw = true;
    }
  ...
  test('snapshotWorktreeInventory returns ok:false with reason on timeout, not throw', () => {
    ...
    let result;
    try {
      result = snapshotWorktreeInventory('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });
    } catch {
      threw = true;
    }
  ...
  test('resolveWorktreeContext returns a valid result on timeout, not throw', () => {
    ...
    let result;
    try {
      result = resolveWorktreeContext('/tmp', { vcs: makeFakeVcsTimedOut() });
    } catch {
      threw = true;
    }
...
describe('bug-3281 AC2+AC4: timedOut is a first-class field in results', () => {
  test('planWorktreePrune reason is git_timed_out when execGit returns timedOut:true', () => {
    ...
    const result = planWorktreePrune('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });
  ...
  test('listLinkedWorktreePaths reason is git_timed_out when execGit returns timedOut:true', () => {
    ...
    const result = listLinkedWorktreePaths('/tmp', { readPorcelain: makeReadPorcelainTimedOut() });
  ...
  test('snapshotWorktreeInventory reason is git_timed_out on timeout', () => {
    ...
    const result = snapshotWorktreeInventory('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });
...
describe('bug-3281 AC3: worktree prune flow is non-crashing under degraded git', () => {
  test('full prune flow (plan -> execute) completes without throwing on timeout', () => {
    ...
    try {
      const plan = planWorktreePrune('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });
      executeWorktreePrunePlan(plan, { vcs: makeFakeVcsTimedOut() });
    } catch {
      threw = true;
    }
  ...
  test('inspectWorktreeHealth findings is empty array (not undefined) on timeout', () => {
    ...
    const result = inspectWorktreeHealth('/tmp', {}, { readPorcelain: makeReadPorcelainTimedOut() });

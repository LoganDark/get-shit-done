...
// jj-fork re-baseline (Phase 18 precedent): execute-phase.md carries the
// fork's deliberate cross-backend assert_clean_wc gates + fan-in envelope
// machinery (~99.5 KB; extraction blocked — ~50 tests pin these blocks
// in-file) plus the launcher jj-leg tax (+31 B per embedded workflow).
// VCS-audit 2026-07-08: +1 KiB for the cross-backend #48 execute_waves guard
// (dual git/jj root ladder + dispatched-workspace refusal leg) and the
// enforced git-substrate gate on the branching block.
const XL_CAP = 101376;      // 99 KiB (upstream: 96 KiB)
...
describe('SIZE: discuss-phase progressive disclosure (#717 byte budget)', () => {
  ...
  // jj-fork re-baseline: +31 B launcher jj-leg tax on the embedded shim
  // (permanent divergence; see gsd-fork merge ledger).
  const DISCUSS_PHASE_TARGET = 32100;

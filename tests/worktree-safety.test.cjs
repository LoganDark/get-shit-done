...
describe('worktree record-agent — real CLI dispatch (#1298)', () => {
  ...
  test('the execute-phase.md per-agent append rides the dispatch envelope (19-12 re-point)', () => {
    // 19-12 re-point: Phase 11 D-01 retired the manifest machinery the
    // record-agent verb writes to — the jj fork's per-agent append is the
    // ParallelAgentResult accumulation on $RESULTS_ACCUM, preferring the
    // executor-returned <worktree_metadata> block (#1297/#1298 class) over
    // harness metadata. The record-agent CLI verb itself remains covered by
    // the write-strict envelope tests above.
    const wf = fs.readFileSync(
      path.join(__dirname, '..', 'gsd-core', 'workflows', 'execute-phase.md'), 'utf8',
    );
    assert.match(wf, /RESULTS_ACCUM/, 'execute-phase.md must accumulate ParallelAgentResult entries');
    assert.match(wf, /<worktree_metadata>/, 'execute-phase.md must consume executor-returned worktree metadata (#1297)');
  });
});

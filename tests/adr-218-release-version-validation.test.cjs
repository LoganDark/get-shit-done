...
// jj-fork runtime skip guard (T-19-02 disposition): release.yml is org
// automation carrying org secrets, dropped from the fork in 19-01/phase-19
// security remediation. Gate every describe below on its presence (no
// top-level return — espree-based scanners parse this file as a script).
const WORKFLOW_PRESENT = fs.existsSync(WORKFLOW_PATH);
if (!WORKFLOW_PRESENT) {
  test('skipped — release.yml dropped from the fork (T-19-02; ledgered)', () => {});
}

WORKFLOW_PRESENT && describe('ADR-218 — leading-zero rejection regex (behavioral)', () => {
...
WORKFLOW_PRESENT && describe('ADR-218 — structural wiring of release.yml', () => {

...
describe('quick: pre-dispatch worktree base re-check (#1941)', () => {
  ...
  test('Step 6 runs worktree.base-check before capturing EXPECTED_BASE', () => {
    ...
    // 19-12 re-point: the jj fork captures EXPECTED_BASE through the adapter
    // (`gsd_run query head-ref --cwd . --pick head`), not raw git rev-parse.
    const expectedBaseIdx = content.indexOf('EXPECTED_BASE=$(', step6Idx);

...
describe('bug #3212 execute-phase stall detection and safe resume', () => {
  ...
  test('execute-phase verifies partial-plan drift before dispatch', () => {
    ...
    // VCS-audit 2026-07-08 re-point: the plan-commit probe is adapter-mediated —
    // `gsd_run query log --max-count 30` jq-filtered on CURRENT_PLAN_ID in the
    // subject replaces raw `git log --oneline --grep`.
    assert.match(workflow, /gsd_run query log --max-count 30[\s\S]{0,160}?CURRENT_PLAN_ID/, 'safe resume gate must check commits for the current plan id');

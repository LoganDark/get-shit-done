...
describe('pr-target-validator.yml structural wiring', () => {
  if (!fs.existsSync(WORKFLOW_PATH)) {
    // jj-fork runtime skip guard (19-01 disposition): pr-target-validator.yml
    // is org automation dropped from the fork; the classifyPrTarget module
    // tests above still run — only the workflow wiring is fork-absent.
    test('skipped — pr-target-validator.yml dropped from the fork in 19-01 (org automation; ledgered)', () => {});
    return;
  }
  let workflowSrc;

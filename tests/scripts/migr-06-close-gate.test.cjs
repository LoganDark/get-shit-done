 ...
 * src/vcs/format-migration/rewrite.cts (19-12: re-pointed from the deleted
 * sdk/ residue at the 19-05 ported counterpart).
 *
 * The close-gate script intentionally duplicates the rewriter's allowlist
 * inline (rewrite is ESM-source-only and the close-gate is a CJS one-shot —
 ...
 *
 * Test strategy: load the canonical set from rewrite.cts via a tsx
 * subprocess (the ported source keeps `import.meta.url`-free CJS shape but
 * tsx handles the .cts transform uniformly), JSON-serialize the names, then
 * assert every canonical key is present in the close-gate's exported Set.
 */
...
test('migr-06-close-gate COMMIT_KEY_ALLOWLIST is a superset of rewrite.cts COMMIT_KEY_ALLOWLIST (WR-02)', () => {
	...
	// from a one-liner; if rewrite.cts ever stops exporting the symbol the
	...
	const canonicalJson = execFileSync(
		...
		[
			'tsx',
			'-e',
			"import { COMMIT_KEY_ALLOWLIST } from './src/vcs/format-migration/rewrite.cts'; process.stdout.write(JSON.stringify([...COMMIT_KEY_ALLOWLIST]))",
		],
	...
	assert.deepEqual(
		...
		`migr-06-close-gate.cjs COMMIT_KEY_ALLOWLIST is missing canonical keys: ${JSON.stringify(missing)}. ` +
			...
			`src/vcs/format-migration/rewrite.cts. Drift in the OTHER direction (close-gate has MORE keys) is fine.`,
	);
});
